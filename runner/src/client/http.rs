...
pub async fn submit_config_via_http(
    server_addr: &str,
    json_str: String
) -> Result<String> {
    // Create HTTP client
    ...
    // Prepare the request
    let response = client.post(&format!("http://{}/submit", server_addr))
    ...
    if response.status() != StatusCode::OK {
        let error_msg = response.text().await.context("Failed to read error response")?;
    ...
    let response_body: Value = serde_json::from_str(&response_text).context("Failed to parse JSON response")?;
